...
## Shared-document field prediction

A document may contain 2–6 `fields` with a shared candidate set. The processor places candidate markers once, followed by one query marker per field. One Qwen forward returns all query representations; the shared head computes a `[batch, fields, candidates]` tensor. Training averages candidate cross entropy across fields. This avoids repeating image encoding within that document request, without changing DeltaNet masks or branching its state.

Queries remain causally ordered: later fields can attend to earlier field instructions, never their labels. This does not promise field-order invariance or equality to independent-question predictions. Use `--shuffle-fields` during training and measure order sensitivity. Document batching with different field counts is not implemented; submit one document per call. Checkpoints retain the same head shapes, but existing single-question weights need multi-field training and evaluation. This is candidate selection, not parallel generation of arbitrary strings.
